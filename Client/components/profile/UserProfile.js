...
const UserProfile = props => {
    ...
    useEffect(() => {
     const fetchUserData = async()=>{
        ...
        setProfile(data)
        const res = await axios.post(constants.uri+'/users/myFavorites', { id: data._id })
    ...
    const removeFromFavorites = async (item) => {
        const res = await axios.post(constants.uri+"/users/remove-from-favorites", { id: profile._id, productId: item.productId })
        if (res.data) {
            //toast.success("Removed from your favorites collection!")
            alert('Removed from your favorites collection!');
            const {data} = await axios.post(constants.uri+'/users/myFavorites', { id: profile._id })
            setProductGrid(data)
        }
    }
    ...
    const onChangeSearchFav = async (e) => {
        ...
        if(e.target.value.length == 0){
            ...
            setSearchFav("")
            const res = await axios.post(constants.uri+'/users/myFavorites', { id: profile._id })
            setProductGrid(res.data)
        }
        else{
        ...
        <Fragment>
            <Card style={{ margin: 20 }}>
                <Card.Body>
                    <Row>
                        <Col sm={3}>
                            <Image rounded width={175} height={150} src={profile?.profileImg ? profile?.profileImg : defaultProfileImg} />
                        </Col>
                        <Col>
                            <Row>
                        ...
                        <Col>
                            <Link to={`/profile/edit`}><i class="far fa-edit"  aria-hidden="true"></i></Link>
                        </Col>
                    </Row>
                </Card.Body>
            </Card>
