...
const NavBarLayout = props => {
  ...
  useEffect(async () => {
    ...
    if (user.data) {
      //LoggedIN
      setLoggedIn(true)
      const { data } = await axios.post(constants.uri + "/order/cart-items", { userId: user.data._id })
      if (data) {
    ...
    <Fragment>
      <Navbar bg="light" variant="dark" expand="lg">
        <Container fluid>
          ...
          {/* <Col sm={1}></Col> */}
          <Col sm={1}>
            <Navbar.Brand href="#"><Link to="/dashboard" style={{ textDecoration: 'none', color: 'black' }}><span style={{ fontSize: 28, fontWeight: 'bold' }}>Etsy</span></Link></Navbar.Brand>
          </Col>
            ...
            >
              ...
              {loggedIn && (
                <>
                  ...
                  <NavDropdown title={(<i style={{color:'black'}} class="fa fa-user-circle" aria-hidden="true"></i>)} id="basic-nav-dropdown" style={{color:'black'}}>
