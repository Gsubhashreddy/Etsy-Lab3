...
const ProductList = () => {
    ...
    useEffect(async () => {
        ...
        if (params.search) {
            ...
            var { data } = await axios.post(constants.uri + `/products/by-category`,{category})
    ...
    const addToFavorites = async (product) => {
        if (favorites.indexOf(product._id) > -1) {
            ...
            if (index != -1) {
                try {
                    const res = await axios.post(constants.uri + "/users/remove-from-favorites", { id: userId, productId: product._id })
                    if (res.data) {
                        fav.splice(index, 1)
                        setFavorites(fav)
            ...
            //Add to favorites
            try {
                const res = await axios.post(constants.uri + "/users/add-to-favorites", { id: userId, productId: product._id })
